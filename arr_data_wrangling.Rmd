...
outreach <- read_csv(here("outreach_data.csv")) # download from "OPP Arrearages Correspondence" spreadsheet on SP, only save these columns (updated 11/12)

replace_emails <- read_csv(here("bounced_email.csv")) # emails that were undeliverable (Oct outreach)

survey_1A <- read_csv(here("1A_status.csv")) # combined into one spreadsheet and updated 11/12
...
started_app <- read_csv(here("app_started.csv")) %>% # updated 11/12 - from David 
  select(PWSID
         #, funds_disbursed = "Fund Disbursed"
         ) %>% 
  mutate(app_started = "Yes")

submitted_app <- read_csv(here("app_submitted.csv")) %>% #updated 11/15 - comes from exported dashboard data; rename columns and add additional for completed
...
all_program_data <- left_join(survey_data, DACs, by="PWSID") %>% 
...
public_data <- all_program_data %>% 
  ...
  select("Water System ID # (PWSID)"=PWSID, 
         ...
         "County"=county, 
         "# of Service Connections" = service_cxns_update, 
